...
import { FabricOrder, MaterialOrder, PaginatedData } from '@/lib/types';
...
function MaterialOrdersTable({ data }: { data: PaginatedData<MaterialOrder> }) {
      ...
      cell: ({ row }) => (
        <div className="-ml-3 flex h-full w-full gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span>{row.original.material.color}</span>
            <span className="mt-2 text-xs text-muted-foreground">
              {t('grammage')}
            </span>
            <span>{row.original.material.size}</span>
            <span className="mt-2 text-xs text-muted-foreground">
              {t('type')}
            </span>
            <span>{row.original.material.type}</span>
          </div>
        </div>
      )
    },
      ...
      cell: ({ row }) => {
        return (
          <div className="flex flex-col">
            ...
            <span className="mb-2">
              {`${row.original.orderAmount} ${row.original.material.orderUnit}`}
            </span>
            {status === '3' && (
              <>
                <span className="text-xs text-muted-foreground">
                ...
                <span className="mb-2">
                  {`${row.original.incomingAmount} ${row.original.material.orderUnit}`}
                </span>
              </>
            )}
            <span className="text-xs text-muted-foreground">
        ...
        return (
          <div className="float-end -mr-3 flex flex-col gap-2">
            <Button
              ...
              size="sm"
              onClick={(e) => {
                router.push(`/material/order/${row.original.id}`);
              }}
            >
              {t('manage_order')}
            </Button>
  ...
  return (
    <>
      <ConfirmDeleteDialog
        ...
        endpoint="/MaterialColorVariantOrders"
        mutationKey={['delete-material-order']}
        state={deleteState}
      ...
      <div className="flex items-center justify-between">
        ...
        <Badge className="inline-block rounded">{`${t('total_order')}: ${
          data.count
        }`}</Badge>
      </div>
      <DataTable
