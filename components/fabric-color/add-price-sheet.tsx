...
function AddPriceSheet() {
  ...
  const nameProperty = isMaterial
    ...
    : 'fabricSupplierName';
  const idProperty = isMaterial ? 'materialSupplierId' : 'fabricSupplierId';
  ...
  const supplierColors = useQuery({
    queryKey: ['fabric-supplier-colors'],
    queryFn: async () => {
  ...
  console.log(supplierColors.data);
      ...
      <SheetContent>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <FormField
              control={form.control}
              name="fabricSupplierFabricColorId"
              render={({ field }) => (
                <FormItem>
